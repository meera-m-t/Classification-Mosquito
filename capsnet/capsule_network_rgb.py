...
class CapsuleNet(nn.Module):
    def __init__(self):
        ...
        self.primary_capsules = CapsuleLayer(num_capsules=4, num_route_nodes=-1, in_channels=256, out_channels=32,
                                             kernel_size=9, stride=2)
        self.digit_capsules = CapsuleLayer(num_capsules=NUM_CLASSES, num_route_nodes=73728, in_channels=4,
                                           out_channels=16)
...
if __name__ == "__main__":
    ...
    def get_iterator(mode):
        ...
        if mode == True:
            ...
            kwargs = {'num_workers': 1, 'pin_memory': True} if cuda else {}
            data_set = torch.utils.data.DataLoader(dataset, batch_size=556, shuffle=False, **kwargs)   
            data_iter = iter(data_set)
            data, labels = data_iter.next() 
